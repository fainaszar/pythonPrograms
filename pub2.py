...
for member in tags:
	...
	if count == 0: 
		...
		#AuthorList = member.find('MedlineCitation')
		...
		#CitationSubset = citation.find('CitationSubset')
		...
		#Abstract = article.find('Abstract').tag
		#AbstractText = article.find('AbstractText')
	...
	try:
		for QualifierName in MeshHeading.findall('QualifierName'):
			#Mesh.append(Author[0].text+ " " + Author[1].text)
