...
class AuthorCreate(CreateView):
	model = Author
	fields = '__all__'
	initial = {'date_of_death':'12/10/2050',}
...
class BookUpdate(UpdateView):
	model = Book
	fields = ['title','summary','genre','language']
...
def atest(request):
	...
	data = req.get('http://swapi.co/api/people/7')
	
	op = data.json()

	from django.contrib.staticfiles.storage import staticfiles_storage

	url = staticfiles_storage.url('output.json')

	# print(url)
	
	data = open("catalog/static/op2.json").read()
	data = str(data)
	import ast
	...
	return render(request,'catalog/atest.html',{
		"titles": out["PubmedArticleSet"]["PubmedArticle"]
		})